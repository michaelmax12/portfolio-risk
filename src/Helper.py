...
class HELPER():
    ...
    def get_market_caps(self, df):
        ...
        Falls back to equal weighting if rate-limited or data is unavailable.
        ...
        # Try batch download first (much less likely to trigger rate limits)
        try:
            ...
            for c in tickers:
                ...
                if shares and last_price:
                    ...
                    mcaps[c] = mcap
                    log.warning(f"Used marketCap fallback for {c}: {mcap}")
                else:
                    mcaps[c] = None
                    log.warning(f"Could not get market cap for {c}")

            # If batch worked for everything, return
            if all(v is not None for v in mcaps.values()):
                return mcaps
        except Exception as e:
            log.warning(f"Batch fast_info failed: {e}. Falling back to per-ticker lookup.")

        # Per-ticker fallback with retry + delay
        for c in tickers:
            if mcaps.get(c) is not None:
                continue  # already have it from batch
            last_price = df[c].iloc[-1]
            for attempt in range(3):
                try:
                    ...
                    if shares and last_price:
                        ...
                        mcaps[c] = None
                        log.warning(f"No market cap data for {c}")
                    time.sleep(0.5)
                    break
                except Exception as e:
                    wait = 2 ** attempt * 2
                    log.warning(f"Rate limit / error for {c} (attempt {attempt+1}): {e}. Waiting {wait}s.")
                    time.sleep(wait)
            else:
                mcaps[c] = None
                log.warning(f"Failed to fetch market cap for {c} after retries.")

        # Final fallback: equal weight any tickers still missing data
        missing = [c for c, v in mcaps.items() if not v]
        if missing:
            log.warning(f"Using equal-weight fallback for: {missing}")
            valid_caps = [v for v in mcaps.values() if v]
            avg_cap = np.mean(valid_caps) if valid_caps else 1.0
            for c in missing:
                mcaps[c] = avg_cap
